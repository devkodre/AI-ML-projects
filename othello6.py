...
#args = ['o..x...xxoooxooooxoooxo..xoxo.o..oxxooooooxxxoxxooxxoox.oooooox.', 'x', 'HL13']
...
def main():
    ...
    if not possibleChoices:
        ...
        possibleChoices, flippedSection = findMoves(nb,token)
    #No move made B-E
    if (not moves and verbose) or not verbose:
    ...
    for move in moves:
        if move >= 0:
            ...
            if not possibleChoices:
                token, opT = opT, token
                possibleChoices, flippedSection = findMoves(nb,token)
            if (move == moves[len(moves)-1] and verbose) or not verbose:
    ...
    if possibleChoices:
        ...
        if nb.count(".") < hl:
            ab =alphabeta(nb, token, -65,65, abCache)
...
def alphabeta(brd,tkn, alpha, beta, abCache):
    eTkn = "XO".replace(tkn,"")
    if "." not in brd: 
        return [brd.count(tkn)-brd.count(eTkn)]
    possibleMoves, flippedSections= findMoves(brd,tkn)
    ...
    if not possibleMoves:
        ...
        if (brd, eTkn,-beta,-alpha) not in abCache:
            abCache[(brd, eTkn,-beta,-alpha)] = alphabeta(brd,eTkn,-beta,-alpha,abCache)
    ...
    bestSoFar = [alpha-1]
    for mv in orderMoves(brd,tkn,possibleMoves):
        newBrd = makeMove(brd,tkn,mv,flippedSections)

        if (newBrd, eTkn,-beta,-alpha) not in abCache:
            abCache[(newBrd, eTkn,-beta,-alpha)] = alphabeta(newBrd,eTkn,-beta,-alpha,abCache)
