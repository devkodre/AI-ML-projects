...
def playGame(quickMove, token, show=False):
  ...
  while True:
    ...
    moves, fs = findMoves(brd, tknToPlay)
    if not (moves):
      tknToPlay = 'xo'[tknToPlay=='x']
      moves, fs = findMoves(brd, tknToPlay)    # Swap players if pass
      if not (moves): break
    ...
    brd = makeMove(brd, tknToPlay, mv, fs)      # , moves   
  ...
  if show: print(f"\nScore: Me as token {token}: {tknCt} vs Enemy: {enemy}\n")
...
# Parse args here
tkn, trnyCt, defFile = "x", 100, "othello7"    # token, game count, users file name
for arg in args:
