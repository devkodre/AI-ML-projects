...
def main():
    ...
    #with open(args[0]) as f1:
      ...
      puzzle = "KHBILDMEANJG_FCO"

      #if i == 0: 
      goal = "DBAELFKJGNCM_IOH"
      pzlen = len(goal)
...
def solve(puzzle, goal):
  if puzzle == goal:
    print(puzzle + " : " + condensePath([puzzle], puzzle))
    return 0
  if not isSolvable(puzzle, goal):
    print(puzzle + " : " + condensePath([], puzzle))
    return -1
...
def h(pzl, goal):
  ...
  for x in range(len(pzl)):
    for y in range(len(pzl)):
      if pzl[x] == goal[y]:
        manhattanSum += abs(x/w - y/w) + abs(x%w - y%w)
  return manhattanSum
