...
args = '.**oooo..*oxxo**xxxxoooo.xxxoxoooxxoxoxoooxxoxooo*.xxo*o...oooo.'.replace("*",".").split(" ")
#args = '192029'.split(" ")
...
def alphabeta(brd,tkn, alpha, beta, abCache,n=5,timeLeft = 0, secondBest = False):
    ...
    if n <= 0: 
        if timeLimit < 5:
            return [evaluate(brd,tkn,timeLimit)]
        t= time.time()
...
def evaluate(brd, tkn, tl,ifImported = False):
    ...
    WEIGHTS = [-35, 0*78.922, 501.724, 0, 45, 274.396, 30, 0*10,
                -20, 58.922, 501.724, 0, 15, 54.396, 30, 0*10,
                -5, 78.922, 501.724, 5,0, 74.396, 30, 0*20,
                0,  78.922, 501.724, 15, 0, 74.396, 30, 0*20]
    ...
    score = (cpScore*w1 + mobScore*w2 + cScore*w3 + eScore*w4 + msScore*w5 + pmScore*w6 + acScore*w7 + tScore*w8)#tW
...
def quickMove(board, token, time = 0,tl = 1):
#set globals
    ...
    if tl == 1 and dots > hl:
        ...
        #    i = 2
        ab = alphabeta(board.upper(), token.upper(), -10000000, 10000000, abCache,i,time)
    ...
    ab = alphabeta(board, token, -10000000, 10000000, abCache,n,time)
    print(ab)
    print(f"n = {n}")
    return ab[-1]
